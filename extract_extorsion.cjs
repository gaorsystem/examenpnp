...
(async () => {
  const q1 = await extractQuestions("https://docs.google.com/forms/d/e/1FAIpQLSfqvtuV5QfcwJT3NnSpZwsUtH5Kaoml3NZj8-gbPbuyLu3i5w/viewform");
  
  // Assign missing numbers if any based on previous or index
  let lastNum = 1400; // starting index if no number
  q1.forEach((q, i) => {
    if (!q.numero) {
      q.numero = lastNum + 1;
    }
  ...
  fs.writeFileSync("./src/data/rawExtorsion.json", JSON.stringify(q1, null, 2));
